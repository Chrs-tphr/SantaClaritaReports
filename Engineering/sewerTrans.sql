SELECT
	...
	,owner.ownerName
	,COALESCE(contra.B1_BUS_NAME,(contra.FNAME||' '||contra.LNAME)) eng
	,tsi.pcNo
	,asitPiv.bondNo
	,asitPiv.bondStat
	,tsi.noticeCompl
...
LEFT OUTER JOIN(
	...
	WHERE
		...
		AND gsi.B1_CHECKBOX_TYPE IN ('SS PLANCHECK','SANITARY SEWER')
		AND gsi.B1_CHECKBOX_DESC IN ('PC No.','Notice of Completion - 95%'
				,'Acceptance LAC Sewer Maintenance','GIS Feature Attached to Case')
		) a
PIVOT(
		MAX(B1_CHECKLIST_COMMENT)
		FOR B1_CHECKBOX_DESC IN ('PC No.' AS pcNo,'Notice of Completion - 95%' AS noticeCompl
				,'Acceptance LAC Sewer Maintenance' AS accptMaint,'GIS Feature Attached to Case' AS gisFt)
		) tsi ON
...
WHERE
	...
	AND b.B1_PER_TYPE = 'Sanitary Sewer'
	AND (
			(
